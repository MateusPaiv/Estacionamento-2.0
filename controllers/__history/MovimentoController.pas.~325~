...
begin
   ...
   if (idMvtCadastroFunc <= 0) or (idMvtManobristaFunc <= 0) then begin
     MessageDlg
      ('Código de veículo ou de funcionário vazio', mtInformation,[mbOK], 0);
      Result := False;
      exit;
   end;
   ...
   if (saidaPrevista>Q.FieldByName('MAX').AsDateTime) or (saidaPrevista=Q.FieldByName('MAX').AsDateTime) then begin
      Aviso
      ('Estacione na frente do veículo com a placa:'+Q.FieldByName('placa').asString);
   end else begin
      Q.SQL.Clear;
      Q:=
      SqlToQuery(
      ...
      while not Q.Eof do begin
         ShowMessage(TimeToStr(Q.FieldByName('min').AsDateTime));

         if (saidaPrevista<Q.FieldByName('min').AsDateTime) or (saidaPrevista=Q.FieldByName('min').AsDateTime) then begin
            Aviso('Estacione atrás do veículo com a placa:'+Q.FieldByName('placa').asString);
            break;
         end;
